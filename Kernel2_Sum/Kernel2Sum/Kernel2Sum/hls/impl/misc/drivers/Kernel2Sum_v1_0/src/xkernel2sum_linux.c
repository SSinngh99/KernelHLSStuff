...
typedef struct {
    ...
    XKernel2sum_uio_map maps[ MAX_UIO_MAPS ];
} XKernel2sum_uio_info;

/***************** Variable Definitions **************************************/
static XKernel2sum_uio_info uio_info;
...
static int uio_info_read_name(XKernel2sum_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XKernel2sum_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XKernel2sum_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XKernel2sum_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XKernel2sum_Initialize(XKernel2sum *InstancePtr, const char* InstanceName) {
	XKernel2sum_uio_info *InfoPtr = &uio_info;
